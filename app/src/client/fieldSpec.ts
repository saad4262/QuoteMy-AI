    ...
    docHints: {
      values: [
        ...
        [
          ...
          /* `extras` is a multiEnum and collects EVERY pattern that fires, so the bare word has to
             stand off the storage cupboard that shares it: an appliance garage is a place to put a
             kettle, not cabinetry cut to take a dishwasher. */
          /\bappliances?\b(?!\s?garage)|\bdishwashers?\b|\bovens?\b|\bcook\s?tops?\b|\brange\s?hoods?\b|\bintegrated\b/i,
        ],
        ['sink', /\bsinks?\b/i],
        ['laundry', /\blaundr(?:y|ies)\b/i],
        ['appliance_garage', /\bappliance\s?garages?\b/i],
        ['open_shelving', /\bopen\s?shel(?:f|ves|ving)\b/i],
        ['pull_out_bin', /\bpull[-\s]?out\s?bins?\b|\bbin\s?pull[-\s]?outs?\b/i],
        ['corner_storage', /\bcorner\s?(?:storage|units?|carousels?)\b|\blazy\s?susans?\b/i],
      ],
    },
  },
  {
